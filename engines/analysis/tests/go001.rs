...
/// A work package must never mix units, and the IIM rewrite must stay at 2.
/// Summing quantities across findings once turned 2 IIM rewrites plus 11
/// indirect references into a single 13-unit line, which an estimating template
/// then priced as thirteen rewrites.
#[test]
fn work_packages_never_mix_units() {
    let (r, _) = run();
    let wps = r["work_packages"].as_array().unwrap();

    let rewrite: Vec<&Value> = wps
        .iter()
        .filter(|w| w["code"] == "UNSUPPORTED_INSTRUCTION_REWRITE")
        .collect();
    let instr = rewrite
        .iter()
        .find(|w| w["unit_type"] == "INSTRUCTION")
        .expect("the IIM rewrite line is missing");
    assert_eq!(
        instr["quantity"].as_u64(),
        Some(2),
        "the IIM rewrite is 2 instructions, not the sum of every finding that shares the package"
    );
    assert!(
        rewrite.iter().any(|w| w["unit_type"] == "REFERENCE"),
        "indirect references must be their own line, not folded into instructions"
    );
